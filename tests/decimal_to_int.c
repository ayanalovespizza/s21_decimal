...
//    s21_decimal result = {0};
//    // Предположим, что первые 16 бит bits[3] используются для хранения значения scale.
//    result.bits[0] = 123456789; // Например, 123456789.987
//
//    int res = 0;
//    int success = s21_from_decimal_to_int(result, &res);
//    ck_assert_int_eq(success, 0); // или ck_assert_int_eq(success, 1), если функция должна сообщать об ошибке
//    ck_assert_int_eq(res, 123456789); // или другое ожидаемое поведение
//} END_TEST
//
//
...
Suite *decimal_to_int(void) {
    Suite *s = suite_create("\033[45m-=S21_from_decimal_to_int_mini=-\033[0m");
